...
flowchart TD
    ...
    A --> C[Apresentação da Turma]
    C --> C1[Experiências profissionais]
    C --> C2[Expectativas após o curso]
    ...
    F --> F2[Projetos: Educação Financeira]

    A --> G[Sistemas Operacionais]
    G --> G1[Definição: gerencia hardware e software]
    G --> G2[Importância: interface usuário-máquina]
    G --> G3[Exemplos: Windows, macOS, Linux, Android, iOS]

    A --> H[Estrutura Interna]
    H --> H1[Estrutura em camadas]
    H --> H2[Kernel: monolítico vs modular]
    ...
    J --> J2[Memória virtual: paginação, segmentação, TLB, swap]
    ...
    L --> L2[Virtualização: hipervisores, containers, orquestração]
    L --> L3[Tendências: NFV, serverless, IoT/edge]

    A --> M[Portfólio de Projetos]
    M --> M1[README claro]
    M --> M2[Instruções de execução]
    M --> M3[Código organizado]
    M --> M4[Documentação técnica]
    M --> M5[Licença]
